#!/usr/bin/env python3
"""
Optimized image anonymization script with more parameter adjustment options 
for achieving the best anonymization results
"""
import cv2
import torch
import tops
...
def resize_to_original(anonymized_image, original_size):
    """Resize anonymized image back to original size"""
...
def anonymize_image(image_path, output_path, anonymizer, 
    ...
    Parameters:
    - truncation_value: Truncation value, controls generation diversity (0.0-1.0, lower=more conservative)
    - multi_modal_truncation: Whether to use multi-modal truncation
    - preserve_original_size: Whether to preserve original image size
...
def main(config_path, input_path, output_path, visualize, detection_score_threshold, 
    ...
    """
    Optimized image anonymization tool
    
    Parameter tuning recommendations:
    
    1. detection_score_threshold (Face detection threshold):
       - 0.1-0.3: Detect more faces, including blurry/small faces
       - 0.3-0.5: Balance detection accuracy and recall (recommended)
       - 0.5-0.9: Only detect clear/large faces
    
    2. truncation_value (Truncation value):
       - 0.3-0.5: Very conservative, more realistic faces but lower diversity
       - 0.6-0.8: Balance realism and diversity (recommended)
       - 0.8-1.0: More diverse but may be less realistic
    
    3. multi_modal_truncation:
       - False: Standard truncation (recommended for most cases)
       - True: Multi-modal truncation, increases diversity
    
    4. quality:
       - 85-95: High quality output (recommended)
       - 95-100: Highest quality but larger files
    """
