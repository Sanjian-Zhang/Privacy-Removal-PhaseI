...
import shutil
            try:
                ...
                logger.error(f"❌ RetinaFace初始化失败: {tf_error}")
                ...
                try:
                    ...
                    pass  # 忽略清理错误
                RetinaFace.detect_faces(test_img)
                logger.info("✅ RetinaFace模型重新初始化成功")ib import Path
from typing import Dict, List, Tuple, Optional
from tqdm import tqdm
import logging
import gc
from collections import defaultdict
import torch  # 添加torch导入用于GPU检查

# 强制使用CPU模式，避免CUDA初始化错误
...
# === 参数配置 ===
# 目录配置
INPUT_DIR = '/home/zhiqics/sanjian/predata/output_frames48'
OUTPUT_BASE_DIR = '/home/zhiqics/sanjian/predata/output_frames48'
PLATE_MODEL_PATH = '/home/zhiqics/sanjian/predata/models/license_plate_detector.pt'

# 新计分系统阈值
SCORE_THRESHOLD = 5                # 总分阈值（>4分符合要求）
CLEAR_FACE_SCORE = 2              # 清晰正脸得分
CLEAR_PLATE_SCORE = 2             # 清晰车牌得分
TEXT_RECOGNITION_SCORE = 2        # 文字识别得分（10个字段得2分）
TEXT_FIELDS_THRESHOLD = 10        # 文字字段数量阈值
...
# GPU加速配置
USE_GPU = False                       # 禁用GPU加速（避免CUDA错误）
GPU_DEVICE_ID = 0                     # 使用的GPU设备ID (0 或 1)
ENABLE_TORCH_OPTIMIZATION = False     # 禁用PyTorch优化
...
class FacePlateClassifier:
        ...
        try:
            if USE_GPU and torch.cuda.is_available():
                ...
                if GPU_DEVICE_ID < device_count:
                    ...
                    gpu_memory = torch.cuda.get_device_properties(GPU_DEVICE_ID).total_memory / 1024**3
                    logger.info(f"🚀 GPU加速已启用: {gpu_name} (设备 {GPU_DEVICE_ID})")
                    ...
                    logger.warning(f"⚠️  指定的GPU设备ID {GPU_DEVICE_ID} 超出范围，共有 {device_count} 个GPU")
                    return 'cpu'
            else:
    ...
    def _initialize_ocr(self):
        ...
        try:
            ...
            self.ocr_reader = easyocr.Reader(
                ['ch_sim', 'en'],  # 支持中文简体和英文
                gpu=gpu_enabled    # 启用GPU加速
            )
            
            if gpu_enabled:
                logger.info("✅ EasyOCR模型初始化成功 (GPU加速)")
            else:
    ...
    def classify_image(self, image_path: str) -> Tuple[str, Dict]:
        ...
        try:
            ...
            # 可识别文字：达到10个字段得2分
            if text_count >= TEXT_FIELDS_THRESHOLD:
                text_score = TEXT_RECOGNITION_SCORE
                score += text_score
                score_details.append(f"可识别文字 {text_count} 个字段(>={TEXT_FIELDS_THRESHOLD}) = {text_score} 分")
            elif text_count > 0:
                score_details.append(f"可识别文字 {text_count} 个字段(<{TEXT_FIELDS_THRESHOLD}) = 0 分")
    ...
    def print_final_statistics(self):
        ...
        logger.info(f"  - 可识别文字{TEXT_FIELDS_THRESHOLD}个字段 = {TEXT_RECOGNITION_SCORE} 分")
    ...
    def run(self):
        ...
        logger.info(f"  - 可识别文字: {TEXT_RECOGNITION_SCORE} 分(需达到{TEXT_FIELDS_THRESHOLD}个字段)")
