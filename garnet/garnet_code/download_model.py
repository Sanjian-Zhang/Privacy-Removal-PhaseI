#!/usr/bin/env python3
"""
Script to download GaRNet pre-trained model for Docker
"""

import os
from huggingface_hub import hf_hub_download

def download_garnet_model():
    print("Downloading GaRNet pre-trained model from Hugging Face...")
    
    try:
        model_path = hf_hub_download(
            repo_id="naverpapago/garnet",
            filename="saved_model.pth",
            local_dir="/workspace",
            local_dir_use_symlinks=False
        )
        
        # Rename to expected location
        os.rename(model_path, "/workspace/garnet_model.pth")
        print(f"✓ Model downloaded successfully to: /workspace/garnet_model.pth")
        return True
        
    except Exception as e:
        print(f"✗ Failed to download model: {e}")
        return False

if __name__ == "__main__":
    success = download_garnet_model()
    if success:
        print("Model download completed!")
    else:
        print("Model download failed. Please check your internet connection.")
        exit(1)
